...
Let's start \todo{Parece un poco informal usar Let's start, usa otra frase o simplemente sácala} 
by defining Reinforcement Learning and classic solutions methods, and then
...
and the \textbf{algorithms} that solve problems in this framework. RL as a learning approach \todo{mejor decir directamente que is a learning paradigm like}
is a learning paradigm , like supervised and unsupervised learning, 
\todo{Aqui colocar un punto y comenzar con Unlike learning from ... en vez de with the difference...} 
...
We have already talked \todo{discussed} about some of the components of an MDP (state, action, reward).
...
distribution, and a discount factor $\gamma$ that models that \todo{that .. that no suena muy bien} rewards in the future might have
...
the underlying representation of the agent actions we can apply by using an appropiate \textbf{actuation model}.

\subsection{Physics Engines}

The physics engine is the component in charge of simulating the environment by doing the
required collision checkings and applying the dynamics to the simulated objects. These
...
The changes are made based on some tradeoffs that depend on the application. That's
...
\begin{itemize}
    \item \textbf{MuJoCo} [@CITE], developed by Emo Todorov, is a recent physics engine
          used for robotics applications, and widely used for RL locomotion benchmarks through
          various python packages made by various researchers, like Deepmind's controlsuite [@CITE],
          Berkeley's Rllab, OpenAI's mujoco-py, etc. This engine is closed source and it
          requires a license for continuous use.

    \item \textbf{Bullet} [@CITE], developed by Erwin Cowmans, is a very popular opensouce
          physics engine. It provides both C/C++ and Python APIs, and it's been used in packages
          like OpenAI's Roboschool [@CITE], and in research in [@CITE,@CITE].

    \item \textbf{PhysX} [@CITE], developed by NVIDIA, is a physics engine used
          mostly in games, and integrated into game engines like Unity [@CITE] and Unreal [@CITE].
          It has recently been released as opensource, and version 4.0 has new features
          that allow to simulate robotics applications more accurately.

    \item \textbf{FleX} [@CITE], developed also by NVIDIA, is a GPU particle-physics engine,
          ...
          that is posible to simulate large scenes easily and with very few compute resources.
          For example, in [@CITE] the authors (also from NVIDIA) developed a simulator based on
...
some of them related to computing forces and torques using the Featherstone algorithm [@CITE],
...
For more details in actuation models, see [@CITE]. Some physics engines don't support 
various of this actuation models, leaving the user with the responsability to implement
these models using the primitives provided, which usually consist in torques (like the authors
of [@CITE] did in its implementation). Figure @FIG-2.15 shows these actuation models 
(adapted from [@CITE]).
