...
\begin{itemize}
    ...
    \item \textbf{Network architecture}: the actors and critics for each
          ...
          For training the authors chose to use the CACLA algorithm, developed by \cite{CACLA}.
...
\begin{itemize}
    ...
    \item \textbf{Learning}: the learning algorithm used consist of a variant of
          the CACLA algorithm \citep{CACLA2} used in the previously discussed work.
...
\begin{itemize}
    ...
    \item \textbf{Complex terrain and curricula usage}: the authors also used more complicated
          terrain (comparared to flat terrain benchmarks), and created them using procedural
          terrain generation (see Figure ~\ref{fig:ch3_emergence_of_locomotion_terrains}). 
    ...
    \item \textbf{Learning from scratch}: one of the key features of this work is
          ...
          nor motion reference, compared to the related works discusses previously.
          However, because no reference is given the resulting policies give resulting
          locomotion behaviours that are idiosincratic, and in some cases even funny (see
